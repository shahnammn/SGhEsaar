...
function hms(x) {
    x = Math.floor(3600 * x);
    h = Math.floor(x / 3600);
    h1 = Math.floor(x / 3600) +1;
    mp = x - 3600 * h;
    m = Math.floor(mp / 60);
    s = Math.floor(mp - 60 * m);
    return (((h < 10) ? "0" : "") + h1.toString() + ":" + ((m < 10) ? "0" : "") + m.toString() + ":" + ((s < 10) ? "0" : "") + s.toString());
}

function hhh(x) {
    x = Math.floor(3600 * x);
    h = Math.floor(x / 3600) +1;
    mp = x - 3600 * h;
    m = Math.floor(mp / 60);
    s = Math.floor(mp - 60 * m);
...
function shownow() {
    ...
    if (azan_ttt.getTime() > today.getTime()) {
        ...
        if (azan_ttt.getTime() == today.getTime()) {
            ...
            offshownow();
            document.getElementById("azan_p1").src = "/images/flasher.gifflasher.gif";
